...
SUPPORTED_OS = {
  "debian"   => {box: "debian/strech64", bootstrap_os: "debian", user: "vagrant"},
...
$os                     = "ubuntu"
$num_instances          = 1
$instance_name_prefix   = "chartmuseum"
$vm_memory              = 1024
$vm_cpus                = 1
$subnet                 = "10.0.4.1" # For 10.0.4.1X
$box                    = SUPPORTED_OS[$os][:box]

# Ansible provisioner
$playbook               = "chartmuseum.yml"
    ...
    config.vm.define vm_name = "%s%02d" % [$instance_name_prefix, i] do |server|
      ...
      if i == $num_instances
        config.vm.provision "ansible" do |ansible|
          ...
          ansible.groups = {
            "chartmuseum" => ["#{$instance_name_prefix}0[1:#{$num_instances}]"]
          }
        end
      end
    end
